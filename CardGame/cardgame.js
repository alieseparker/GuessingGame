...
function fname() {
  return ("js3e 2/" + this.num + this.suit + ".gif");
}
...
function Deal() {
  ...
  for(var i = 1; i < 6; i++) {
    ...
    document.images[i+5].src = "js3e 2/hold.gif";
    held[i] = false;
  }
  ...
  document.images[11].src="js3e 2/draw.gif";
  Addscore();
}

function Hold(num) {mo
    if (!dealt) {
      return;
    }
    if (!held[num]) {
      held[num] = true;
      document.images[5+num].src="js3e 2/hold2.gif";
    }
    else {
      held[num]=false;
      document.images[5+num].src="js3e 2/hold.gif";
    }
  }
  ...
  function Draw() {
    ...
    dealt = false;
    document.images[11].src = "js3e 2/deal.gif";
    score += Addscore();
    document.form1.total.value = score;
  }
